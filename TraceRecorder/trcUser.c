...
{    
    ...
    for (;;)
    {
        ...
        if (state == STATE_STARTED)
        {
            ...
            {
                sprintf(localsprintbuffer, 
                    "\n\r[FreeRTOS+Trace] Event count: %d, Duration: %ld ms. [%d ticks]\n\r",
                    (int)RecorderDataPtr->numEvents,
                    (int)RecorderDataPtr->absTimeLastEventSecond * 1000 + (RecorderDataPtr->absTimeLastEvent*1000) / RecorderDataPtr->frequency,
